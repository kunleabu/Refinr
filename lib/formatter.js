...
export async function formatBatch(cslJsonItems, styleInput) {
    ...
    try {
        ...
        // opt: suppress et-al truncation so all authors are shown
        const engine = new CiteProc.Engine(sys, cslXml, 'en-US', false);

        // Override et-al settings — show all authors in bibliography
        // (in-text citations still use et al. as per the style)
        if (engine.bibliography && engine.bibliography.opt) {
            engine.bibliography.opt['et-al-min'] = 100;
            engine.bibliography.opt['et-al-use-first'] = 100;
        }
