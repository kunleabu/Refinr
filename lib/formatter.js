...
// Styles that use single quotes for article titles (primarily Harvard variants)
const SINGLE_QUOTE_STYLES = [
...
function patchLocaleForStyle(localeXml, styleId) {
    if (!SINGLE_QUOTE_STYLES.includes(styleId)) return localeXml;
    return localeXml
        .replace(/<term name="open-quote">[^<]*<\/term>/g, '<term name="open-quote">\u2018</term>')
        .replace(/<term name="close-quote">[^<]*<\/term>/g, '<term name="close-quote">\u2019</term>')
        .replace(/<term name="open-inner-quote">[^<]*<\/term>/g, '<term name="open-inner-quote">\u201C</term>')
        .replace(/<term name="close-inner-quote">[^<]*<\/term>/g, '<term name="close-inner-quote">\u201D</term>')
        // Harvard places punctuation outside quotes (British convention)
        .replace(/punctuation-in-quote="true"/g, 'punctuation-in-quote="false"');
}
...
export async function formatBatch(cslJsonItems, styleInput) {
    ...
    try {
        ...
        // Patch locale quotes per style and CSL XML for et-al
        const patchedLocale = patchLocaleForStyle(localeXml, styleId);

        // citeproc-js sys object — provides items and locale to the engine
        const sys = {
            retrieveLocale: (lang) => patchedLocale,
            retrieveItem: (id) => itemRegistry[id]
        };

        // Patch the CSL XML before passing to citeproc:
        // 1. Raise et-al threshold to 99 so all authors show in bibliography
        // 2. Fix punctuation-in-quote — Harvard places punctuation OUTSIDE quotes
        let patchedCsl = cslXml
            .replace(/(<bibliography[^>]*?)et-al-min="\d+"/g, '$1et-al-min="99"')
            .replace(/(<bibliography[^>]*?)et-al-use-first="\d+"/g, '$1et-al-use-first="99"')
            .replace(/(<bibliography)(\s)/g, '$1 et-al-min="99" et-al-use-first="99"$2')
            // Harvard puts punctuation outside quotes (British convention)
            .replace(/punctuation-in-quote="true"/g, 'punctuation-in-quote="false"')
            .replace(/(<style[^>]*?)punctuation-in-quote="true"/g, '$1punctuation-in-quote="false"');

        // Initialise the CSL engine with patched XML
        const engine = new CiteProc.Engine(sys, patchedCsl, 'en-US', false);
