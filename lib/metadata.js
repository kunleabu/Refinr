...
function extractVolume(text) {
    // Strip issue numbers first to avoid confusion
    const withoutIssue = text.replace(/\(\d+\)/g, '');

    const patterns = [
        /\bvol\.?\s*(\d+)/i,
        /\bvolume\s+(\d+)/i,
        // After journal name, before issue: "Nature, 443(7111)" → vol 443
        /[A-Z][a-z]+,\s+(\d{1,4})\s*\(/,
        // "Journal 45(3)" → vol 45
        /[a-z],?\s+(\d{1,4})\s*\(/,
    ];
    for (const pattern of patterns) {
        const match = withoutIssue.match(pattern);
